...
<body class="font-sans antialiased bg-cream text-ink selection:bg-forest-600 selection:text-cream h-screen overflow-hidden">
    <div class="grain-overlay"></div>

    <div class="flex h-full w-full overflow-hidden" x-data="{ sidebarOpen: false }">

        {{-- Backdrop overlay (mobile only) --}}
        <div class="fixed inset-0 bg-black/30 z-20 lg:hidden" x-show="sidebarOpen" x-cloak
             @click="sidebarOpen = false" x-transition.opacity></div>

        {{-- Sidebar --}}
        <aside class="fixed inset-y-0 left-0 z-30 w-64 shrink-0 bg-white/95 backdrop-blur-sm border-r border-sage-200 flex flex-col overflow-y-auto
                      transform transition-transform duration-300 ease-in-out
                      -translate-x-full lg:translate-x-0 lg:relative lg:z-auto"
               :class="{ 'translate-x-0': sidebarOpen, '-translate-x-full': !sidebarOpen }">
            <div class="p-4 md:p-6 border-b border-sage-100 shrink-0 flex items-center justify-between">
                ...
                <button @click="sidebarOpen = false" class="lg:hidden p-1.5 rounded-lg text-sage-400 hover:text-sage-600 hover:bg-sage-100 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>

            <nav class="flex-1 p-3 space-y-1">
                <div class="px-4 py-2 text-[11px] font-semibold uppercase tracking-widest text-sage-400">{{ __('Main') }}</div>
            ...
            <div class="p-3 border-t border-sage-100 space-y-0.5 shrink-0">
                <a href="{{ route('dashboard', ['as' => 'user']) }}"
        ...
        <div class="flex-1 flex flex-col h-full min-h-0 overflow-hidden bg-cream relative">
            ...
            {{-- Header --}}
            <header class="shrink-0 bg-white/80 backdrop-blur-md border-b border-sage-200 px-4 md:px-6 lg:px-8 py-3 md:py-4 z-10">
                <div class="flex items-center justify-between gap-3">
                    <div class="flex items-center gap-3 min-w-0">
                        <button @click="sidebarOpen = !sidebarOpen" class="lg:hidden p-1.5 rounded-lg text-sage-500 hover:text-sage-700 hover:bg-sage-100 transition-colors shrink-0">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
                        </button>
                        <div class="text-sm text-sage-500 font-medium truncate">
                            @yield('breadcrumb', __('Dashboard'))
                        </div>
                    </div>
                    <div class="flex items-center gap-3 md:gap-4 text-sm shrink-0">
                        <span class="text-sage-500 truncate max-w-[120px] md:max-w-none">{{ auth()->user()->name }}</span>
            ...
            <main class="flex-1 overflow-y-auto min-h-0 p-4 md:p-6 lg:p-8">
                @if (session('success'))
                    <div class="mb-4 md:mb-6 flex items-center gap-2.5 bg-green-50 border border-green-200 text-green-700 px-4 md:px-5 py-3 md:py-4 rounded-xl text-sm font-medium shadow-sm">
                ...
                @endif
                @if (session('error'))
                    <div class="mb-4 md:mb-6 flex items-center gap-2.5 bg-red-50 border border-red-200 text-red-700 px-4 md:px-5 py-3 md:py-4 rounded-xl text-sm font-medium shadow-sm">
